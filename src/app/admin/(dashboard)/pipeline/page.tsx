...
import { db } from "@/db";
import { applications, resumeFiles, interviews } from "@/db/schema";
...
export default async function PipelinePage({ searchParams }: { searchParams: Promise<{ jobId?: string }> }) {
  ...
  const indicators = ids.length ? await db.select({ id: applications.id, resume: sql<boolean>`exists(select 1 from ${resumeFiles} where ${resumeFiles.applicationId} = ${applications.id})`, interview: sql<boolean>`exists(select 1 from ${interviews} where ${interviews.applicationId} = ${applications.id} and ${interviews.status} in ('Scheduled','Rescheduled'))` }).from(applications).where(sql`${applications.id} in (${sql.join(ids.map(id => sql`${id}::uuid`), sql`, `)})`) : [];
  const byId = new Map(indicators.map(row => [row.id, row]));
  const columns = results.map((result, index) => ({ status: stages[index], total: result.total, href: `/admin/applications?status=${stages[index]}${jobId ? `&jobId=${encodeURIComponent(jobId)}` : ""}`, cards: result.rows.map(({ application: a, jobTitle }) => ({ id: a.id, name: `${a.firstName} ${a.lastName}`, job: jobTitle, date: a.createdAt.toLocaleDateString("en-US", { month: "short", day: "numeric" }), location: [a.city, a.state].filter(Boolean).join(", "), experience: a.yearsExperience, status: a.status, resume: byId.get(a.id)?.resume ?? false, interview: byId.get(a.id)?.interview ?? false })) }));
