...
/**
 * The full letterhead lockup: mark, "Sohum Systems LLC", and the tagline.
 *
 * The company's own offer letters are issued on a letterhead that carries the
 * wordmark and the tagline "Experience the Expertize" beside the symbol,
 * rather than the stacked mark-over-name used elsewhere. This reproduces that
 * lockup as one inline SVG so the generated letter matches the letterhead a
 * candidate has seen before.
 *
 * The tagline's spelling is the company's own and is deliberate — it is
 * reproduced exactly as it appears on the letterhead, not corrected.
 */
const LETTERHEAD_SVG =
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 880 150">` +
  // Symbol, scaled from its own 286-587 x 0-292 crop into the left slot.
  `<g transform="translate(6 6) scale(0.41) translate(-286 0)">` +
  `<path fill="${FLAME}" d="m428.93,0c-30.41,8.17-142.92,63.88-142.38,116.97,0,58.55,115.04,50.4,143.46,46.7,0,0-14.78-3.13-14.78-19.7s14.78-20.65,14.78-20.65c-30.39-1.79-84.63,6.87-86.35-35.62,0-44.48,79.59-79.3,85.26-87.7Z"/>` +
  `<path fill="${NAVY}" d="m432.65,16.92c-27.2,6.3-63.51,29.24-73.6,56.14-5.03,13.42.9,28.44,14,34.25,22.23,9.85,58.24,5.64,58.24,5.64,0,0-6.46-1.34-6.63-10.42-.17-9.21,6.9-9.07,6.9-9.07-6.25-1.61-46.02,4.6-45.95-22.2.81-27,39.59-48.8,47.03-54.34Z"/>` +
  `<circle fill="${NAVY}" cx="435" cy="143.06" r="10.11"/>` +
  `<path fill="${NAVY}" d="m444.59,286.98c30.41-8.17,142.92-63.88,142.38-116.97,0-58.55-115.04-50.4-143.46-46.7,0,0,14.78,3.13,14.78,19.7s-14.78,20.65-14.78,20.65c30.39,1.79,84.63-6.87,86.35,35.62,0,44.48-79.59,79.3-85.26,87.7Z"/>` +
  `<path fill="${FLAME}" d="m440.87,270.06c27.2-6.3,63.51-29.24,73.6-56.14,5.03-13.42-.9-28.44-14-34.25-22.23-9.85-58.24-5.64-58.24-5.64,0,0,6.46,1.34,6.63,10.42.17,9.21-6.9,9.07-6.9,9.07,6.25,1.61,46.02-4.6,45.95,22.2-.81,27-39.59,48.8-47.03,54.34Z"/>` +
  `</g>` +
  // Wordmark and tagline, set in live text so they stay crisp at print DPI.
  // textLength pins the wordmark to the space it is allotted, so a substituted
  // font can never overflow the viewBox and clip the final letters.
  `<text x="150" y="88" textLength="716" lengthAdjust="spacingAndGlyphs" font-family="Georgia, 'Times New Roman', serif" font-size="70" font-weight="bold" fill="${NAVY}">Sohum Systems LLC</text>` +
  `<text x="866" y="128" textLength="390" lengthAdjust="spacingAndGlyphs" text-anchor="end" font-family="Georgia, 'Times New Roman', serif" font-size="29" font-style="italic" fill="${NAVY}">Experience the Expertize</text>` +
  `</svg>`;

/** The letterhead lockup as a data URI, for the offer letter masthead. */
export const sohumLetterheadDataUri =
  `data:image/svg+xml,${encodeURIComponent(LETTERHEAD_SVG)}`;
